...
void on_write( uv_fs_t * req ) {
  ...
  if( 0 > req->result ) {
    ...
    uv_fs_read( loop, & read_req, open_req.result, & buffer_handle, 1, -1, on_read );
  }
}
...
void on_read( uv_fs_t * req ) {
  ...
  if( 0 > req->result ) {
    // error
    printf( "Error reading %s: %ld %s\n", filepath, req->result, uv_strerror( req->result ) );
...
void on_open( uv_fs_t * req ) {
  ...
  if( 0 > req->result ) {
    ...
    uv_fs_read( loop, & read_req, req->result, & buffer_handle, 1, -1, on_read );
  }
}
...
int main( int argc, char * argv [] ) {
  ...
  do {
    ...
    if( 0 != ( err = uv_fs_open( loop, & open_req, filepath, O_RDONLY, 0, on_open ) ) ) {

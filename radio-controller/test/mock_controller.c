...
int main(int argc, char * argv[])
{
    ...
    while (-1 != (opt = getopt(argc, argv, "f:s:v")))
    {
        switch (opt)
        {
            ...
            default:
                fprintf(
                    stderr,
                    "usage: %s [-f filename] [-s frame size] [-v]\n"
                    "where:\n"
    ...
    if (0 != verbose)
    {
        fprintf(stderr, "mock: radio file: %s\nmock: frame size: %d\n",
                frame_filepath, frame_size);
    }

    if (0 != verbose)
    {
        fprintf(stderr, "mock: setting signal handler\n");
    }
    ...
    do
    {
        if (0 != verbose)
        {
            fprintf(stderr, "mock: allocating read buffer\n");
        }
        ...
        if (0 != verbose)
        {
            fprintf(stderr, "mock: opening file\n");
        }
        ...
        while ((0 == err) && (0 == fin))
        {
            ...
            if (frame_size == bytes_read)
            {
                if (verbose > 0)
                {
                    fprintf(stderr, "mock: processing frame\n");
                }
                bytes_read = 0;
